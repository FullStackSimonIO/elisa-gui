...
export function Terminal({
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between gap-3 border-b border-border/50 bg-black/5 px-5 py-4">
        <div>
      ...
      <div className="flex flex-1 flex-col gap-4 px-5 py-4">
        ...
        >
          {logs.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-2 text-center text-sm text-brand-200/70">
            ...
            logs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  data-terminal-line
                  className="group flex items-start gap-3 rounded-xl border border-transparent bg-white/[0.02] px-3 py-2 transition hover:border-brand/30 hover:bg-white/[0.04]"
                >
                  <span
                    className={cn(
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    <div className="flex flex-wrap items-center gap-2 text-brand-50">
                      ...
                      {log.meta ? (
                        <span className="text-[11px] uppercase tracking-[0.3em] text-brand-200/80">
                          {log.meta}
                        </span>
                      ) : null}
                      <span className="ml-auto text-[11px] text-brand-300/70">
                        {formatTimestamp(log.timestamp)}
                      </span>
                    </div>
                    {log.detail ? (
                      <p className="text-[12px] text-brand-200/80">
                        {log.detail}
                      </p>
                    ) : null}
                    <span className="text-[10px] uppercase tracking-[0.35em] text-brand-300/60">
                      {meta.label}
                    </span>
                  </div>
                </div>
      ...
      <footer className="flex items-center justify-between gap-3 border-t border-border/50 bg-black/10 px-5 py-3 text-xs text-muted-foreground">
