          ...
          {normalizedActions.map((descriptor) => {
            ...
            return (
              ...
              >
                ...
                <Tooltip delayDuration={50}>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className="absolute right-5 top-5 h-18 w-18 rounded-full border border-white/20 bg-white/5 text-muted-foreground backdrop-blur-lg transition hover:border-brand-300/60 hover:text-background hover:dark:text-foreground dark:border-white/10 dark:bg-white/5 dark:hover:bg-accent"
                      onClick={() => handleInfo(descriptor)} // Info-Handler gets executed on Click
                      aria-label={`${label} details`}
                    >
                      <Info className="h-20 w-20" aria-hidden />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent
                    sideOffset={10}
                ...
                <span className="inline-flex h-24 w-24 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-100/70 via-brand-400/35 to-primary/40 text-brand shadow-[0_16px_46px_-42px_rgba(227,55,106,0.5)]">
